...
describe('useLoginUser', () => {
    ...
    it('should successfully log in a user and update states', async () => {
        ...
        mockedApiPost.mockResolvedValueOnce({ data: mockResponseData, status: 200, statusText: 'OK', headers: {}, config: {} });
    ...
    it('should handle Axios errors with a specific backend message', async () => {
        ...
        mockedApiPost.mockRejectedValueOnce(mockAxiosError);
        const { result } = renderHook(() => useLoginUser());
        expect(result.current.loading).toBe(false);
        expect(result.current.error).toBeNull();
        await act(async () => {
    ...
    it('should clear previous errors on new login attempt', async () => {
        ...
        mockedApiPost.mockRejectedValueOnce(mockAxiosError); // First call fails
        const { result } = renderHook(() => useLoginUser());
        ...
        mockedApiPost.mockResolvedValueOnce({ data: mockSuccessResponse, status: 200, statusText: 'OK', headers: {}, config: {} });
